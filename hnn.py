...
class HNN(torch.nn.Module):
    ...
    def time_derivative(self, x, t=None, separate_fields=False):
        ...
        damping_field = torch.zeros_like(x)
        ...
        if self.field_type == 'damping':
            dF2 = torch.autograd.grad(F2.sum(), x, create_graph=True)[0] # gradients for damping field
            damping_field = dF2 @ torch.tensor([[0, -1.0],[1.0, -0.00]]) + self.linear(dF2) @ torch.tensor([[0, -0.0],[0.0, -1.00]])
        
        # if separate_fields:
        #     return [conservative_field, solenoidal_field]

        return conservative_field + solenoidal_field + damping_field
