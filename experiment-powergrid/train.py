...
def get_args():
    ...
    parser.add_argument('--learn_rate', default=2e-3, type=float, help='learning rate')
    parser.add_argument('--batch_size', default=4000, type=int, help='batch_size')
    ...
    parser.add_argument('--name', default='powergrid', type=str, help='only one option right now')
...
def train(args):
  ...
  elif args.field_type == 'damping':
      print("Training nhnn model:")
  else:
  ...
  torch.nn.init.constant_(model.linear.weight, 0.0)
...
if __name__ == "__main__":
    ...
    label = '-nhnn' if args.field_type == 'damping' else label
    # label = '-rk4' + label if args.use_rk4 else label
    # df.to_csv('{}/{}{}.csv'.format(args.save_dir, args.name, label))
