...
class HNN(torch.nn.Module):
  def __init__(self, input_dim, hidden_dim, output_dim, nonlinearity='tanh'):
    ...
    self.linear4 = torch.nn.Linear(input_dim, input_dim, bias=None)
  ...
  def forward(self, x):
    ...
    h = self.nonlinearity( self.linear2(h) )
    h = self.linear3(h)
    dF = torch.autograd.grad(h.sum() ,x, create_graph=True)[0]
    # return self.linear4(torch.tensor(dF))
    return dF @ torch.tensor([[0.0, -1.0], [1.0, -0.0]])
